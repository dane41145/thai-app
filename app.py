...
function updateCardContent() {
    ...
    function setSize(el, text, isThai) {
        ...
        if (isThai) {
            // Thai text - higher thresholds since diacritics don't add width
            if (len <= 3) el.classList.add('text-jumbo');
            else if (len <= 7) el.classList.add('text-huge');
            else if (len <= 12) el.classList.add('text-large');
            else if (len <= 20) el.classList.add('text-med');
            else el.classList.add('text-small');
        } else {
            // English text - higher thresholds to keep single words on one line
            if (len <= 5) el.classList.add('text-huge');
            else if (len <= 12) el.classList.add('text-large');
            else if (len <= 22) el.classList.add('text-med');
            else el.classList.add('text-small');
        }
    }
